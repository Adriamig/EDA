...
template<class T>
inline T Set<T>::getMax()
{
	return elems[size - 1];
}

template<class T>
inline T Set<T>::getMin()
{
	return elems[0];
}

template<class T>
inline void Set<T>::removeMax()
{
	if (size > 0)
		size--;
}
...
template<class T>
Set<T> resolver(Set<T> set, const int K) {
	//int diff = set.sizeSet() - K;
	//if (diff < K) {
	//	if (diff <= 0)
	//		return set;
	//	else {
